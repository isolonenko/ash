...
describe('rtc-store', () => {
  ...
  describe('connect', () => {
    ...
    it('wires all RTCClient event listeners', async () => {
      ...
      const eventNames = mockOn.mock.calls.map(([name]: [string]) => name);
    ...
    it('loads persisted messages from sessionStorage on connect', async () => {
      const persisted = [
        { id: 'msg-1', senderId: 'p1', senderName: 'Bob', text: 'hi', timestamp: 1, fromMe: false },
      ];
  ...
  describe('event handlers → state updates', () => {
    ...
    it('peer-media-state event updates peer audioEnabled and videoEnabled', () => {
      ...
      handler('peer-2', { audioEnabled: false, videoEnabled: false });
